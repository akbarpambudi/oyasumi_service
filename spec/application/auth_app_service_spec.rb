...
RSpec.describe Application::AuthAppService, type: :service do
  ...
  let(:jwt_service)     { class_double(Infrastructure::Services::JwtService) }

  subject(:auth_app_service) { described_class.new(user_repository) }

  let(:name)           { 'John Doe' }
  let(:email)          { '[email protected@email.com]' }
  let(:plain_password) { 'some-password' }

  # This is a simple double for User. We'll allow it to respond to
  # various methods we need, like `authenticate?`, `id`, etc.
  let(:user) do
    instance_double(
  ...
  describe '#sign_in' do
    ...
    context 'when the user is found and password is correct' do
      before do
        ...
        allow(jwt_service).to receive(:encode).and_return(token)

        allow(Infrastructure::Services::JwtService).to receive(:encode).and_return(token)
      end
      ...
      it 'returns a JWT token' do
        ...
        expect(user_repository).to have_received(:find_by_email).with(email)
        expect(Infrastructure::Services::JwtService).to have_received(:encode).with(user_id: user.id)
      end
    end

    context 'when the user is not found' do
      before do
        allow(user_repository).to receive(:find_by_email).with(email).and_return(nil)
      end
  ...
  describe '#authenticate_token' do
    ...
    context 'when the token is valid' do
      before do
        allow(Infrastructure::Services::JwtService).to receive(:decode)
                                                         .with(valid_token)
    ...
    context 'when the user is not found' do
      before do
        allow(Infrastructure::Services::JwtService).to receive(:decode)
                                                         .with(valid_token)
                                                         .and_return({ user_id: 999 })

        allow(user_repository).to receive(:find_by_id).with(999)
